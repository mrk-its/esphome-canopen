...
void CanopenOTAComponent::ota_finish() {
  start_time = millis();
  ota_state = OTA_STATE_WAIT1;
}
...
void CanopenOTAComponent::loop() {
  switch (ota_state) {
    ...
    case OTA_STATE_WAIT1: {
      ...
      start_time = millis();
      ota_state = OTA_STATE_WAIT2;
      break;
    }
    case OTA_STATE_WAIT2: {
      if (millis() - start_time < 1000) {
        break;
      }
      ota_state = OTA_STATE_IDLE;
...
#ifdef OTA_COMPRESSION
  ESP_LOGV(TAG, "offset: %ld, len: %d, data: %02x %02x %02x %02x %02x %02x %02x", this->received, len, data[0], data[1],
           data[2], data[3], data[3], data[4], data[6]);
