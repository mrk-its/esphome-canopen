...
static bool _pred(const CoObj &a, const uint32_t key) { return CO_GET_DEV(a.Key) < CO_GET_DEV(key); };
...
void ObjectDictionary::append(uint32_t key, const CO_OBJ_TYPE *type, CO_DATA data) { od.push_back({key, type, data}); }
